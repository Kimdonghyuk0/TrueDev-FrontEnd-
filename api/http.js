import { getState, updateTokens, clearAuth } from '../state/store.js';
...
async function doFetchWithRefresh(path, options, headers) {
  ...
  if (response.status === 401 && message !== 'invalid_credentials' && !options.suppressUnauthorizedEvent) {
    ...
    if (refreshed) {
      ...
      retryHeaders.Authorization = `Bearer ${getState().token}`;
      ...
      const retryMessage = typeof retryPayload === 'object' && retryPayload?.message
